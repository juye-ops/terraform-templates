...
module "vpc" {
  ...
  onprem = {
    ip_address        = "0.0.0.0" # Need a Modification
    ipv4_network_cidr = "192.168.0.0/24"
  }
}

module "security_group" {
  source = "./modules/security_groups"
  vpc_id = module.vpc.s2s_vpc.id
}
...
module "endpoint" {
  ...
  subnet_ids = [
    module.vpc.s2s_private_subnet.id
  ]
  endpoint_info = {
    ec2messages = {
...
module "ec2_instance" {
  ...
  subnet_id                   = module.vpc.s2s_private_subnet.id
